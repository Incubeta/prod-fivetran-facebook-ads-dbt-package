...
action_values_report as (
	select * 
	from {{ ref('stg_fivetran_facebooks_ads__ad_performance_v_1_action_values') }}
), 
...
outbound_clicks_report as (
	select * 
	from {{ ref('stg_fivetran_facebooks_ads__ad_performance_v_1_outbound_clicks') }}
), 
actions_metrics as (
	select
	_fivetran_synced,
...
metrics_join as (
	...
	on actions_metrics.source_relation = action_value_metrics.source_relation
	...
	on actions_metrics.source_relation = conversions_metrics.source_relation
	...
	on actions_metrics.source_relation = outbound_clicks_metrics.source_relation
	and actions_metrics.action_type = outbound_clicks_metrics.action_type
